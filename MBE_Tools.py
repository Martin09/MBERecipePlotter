...
#import numpy as np
from numpy import greater, bool8, uint8, uint32, float32, float64, ndarray, array, take, frombuffer, empty, hstack, dtype
...
#                    print "stop"
            ...
            if not self.__stop and not self.semaphore.locked():
                try:
                    if not self.connection.sendCommand("OK") == "OK":
...
class ServerConnection:
    ...
    def __connect(self):
        ...
        self.Chamber = self.sendCommand("get this.chamber")
    ...
    def __receiveConfig(self):
        answer=self.sendCommand("get this.config")
...
#        print "received "+str(lengthInfo) + " bytes"
    ...
    def getStatus(self):
        strData=self.sendCommand("get this.StatusInBytes")
